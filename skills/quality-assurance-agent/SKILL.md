...
| **MCP/外部依赖不可用** | MySQL MCP、Playwright MCP、后端 API 不可达 | 可以做降级/模拟检查作为补充证据，但**不得把替代通道当成原层级真实执行**。原 task 若要求浏览器/WinForms/WCF/产品二进制路径而该路径不可用，应标记 blocked；补充 evidence 必须注明 executionMode 与信息损失。 |
